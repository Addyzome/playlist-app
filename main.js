...
function update_second() {
    ...
    if(!isNaN(song[index_no].duration)) {
        position = song[index_no].currentTiem * (100 / song[index_no].duration);
    ...
    let curr_seconds = Math.floor(song[index_no].currentTime - durationMinutes * 60);
    current_duration.textContent = `${curr_minutes}:${curr_seconds < 10 ? '0' : ''}${curr_seconds}`;

    if (song[index_no].ended) {
        clearInterval(update_second) //reset when songs ended
        wave_aniimation.style.opacity = '0';
        play_pause_btn.innerHTML = '<i class="fa fa-play" aria-hidden="true"></i>';
    }
...
//add functionallity to forwars and back arrows
...
backward_btn.addEventListener('click', function(){
    index_no = (index_no - 1) % All_song.length; //movce to previous song
    song[index_no].currentTime = 0;
    play_song();
});
